...
--local tagnames   = { 1, 2, 3, 4, 5, 6, 7, 8, 9, "u", "i", "o", "p" }
...
local weather = lain.widget.weather({
    city_id = 254352, -- placeholder (London)
...
local cpu = lain.widget.cpu({
    settings = function()
        cpu_perc = string.format("CPU % 3d%%", tonumber(cpu_now.usage))
        widget:set_markup(markup("#e33a6e", cpu_perc))
    end
})
-- }}}
...
local memory = lain.widget.mem({
    settings = function()
        mem_perc = string.format("MEM % 3d%%", mem_now.perc)
        widget:set_markup(markup("#e0da37", mem_perc))
    end
})
-- }}}
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, border_width = 0, height = 20 })
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "w", function () weather.show(7) end),                            -- Show weather
    awful.key({ modkey }, "q", function () awful.spawn(browser) end),                       -- Open new browser window
    awful.key({ modkey }, "e", function () awful.spawn(gui_editor) end),                    -- Open editor
    awful.key({ modkey }, "r", function () awful.screen.focused().mypromptbox:run() end,    -- Run program
              {description = "run prompt", group = "launcher"}),

    awful.key({ modkey }, "l", function () calendar.show(7) end),                           -- Show Calendar
    awful.key({ modkey }, "h", function () fswidget.show(7) end),                           -- Show Filesystem space
...
awful.rules.rules = {
    ...
    { rule_any = {
        name = { "Google Hangouts - pmav99@gmail.com" },
        properties = {
            sticky = false,
            floating = true,
            titlebars_enabled = true,
